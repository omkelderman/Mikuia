		...
		switch trigger
			when 'clear'
				if isMod
					...
					Mikuia.Chat.handleResponse data.user.username, data.to, 'Cleared the request list.', data.settings._target, data.details
			when 'delete', 'remove'
				if isMod and tokens.length > 2
					...
					if amountRemoved > 0
						Mikuia.Chat.handleResponse data.user.username, data.to, 'Removed request #' + requestId + '.', data.settings._target, data.details
